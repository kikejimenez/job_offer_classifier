...
__all__ = ['split_dataframe', 'concat_and_shuffle', 'balanced_labels_in_split', 'tf_input_fn', 'train_input_fs',
           'predict_input_fs', 'embedded_text_feature_column_f', 'load_estimator', 'train', 'f1_score', 'evaluate',
           'predict', 'sentiment', 'plot_confusion_matrix', 'export_estimator', 'Pipeline']
...
def split_dataframe(df,**args):
    ...
    '''
    train = df.sample(**args)
    test = df.drop(train.index)
    return {'train': train, 'test': test}

def concat_and_shuffle(train,test,**args):
    df = pd.concat([train,test])
    return df.sample(frac=1.0,**args)

def balanced_labels_in_split(df, **args):
    #df = pd.read_csv(src_file)
    positives = split_dataframe(df[df.sentiment == 1], **args)
    negatives = split_dataframe(df[df.sentiment == 0], **args)
    train = concat_and_shuffle(positives['train'],negatives['train'])
    test = concat_and_shuffle(positives['test'],negatives['test'])
...
class Pipeline:
    ...
    def __init__(self, src_file, estimator_dir=None, frac=0.60):
        self.frac = frac
